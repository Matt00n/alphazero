...
class MinBreakout(environment.Environment):
    ...
    - Must bounce ball to break 3 rows of bricks along top of screen.
    ...
    def step_env(
        ...
        terminal, truncated = self.is_terminal(state, params)
        state = state.replace(terminal=terminal)
        info = {"discount": self.discount(state, params), 'truncation': truncated,}
        return (
    ...
    def is_terminal(self, state: EnvState, params: EnvParams) -> bool:
        ...
        done = state.terminal.astype(float)
        done_steps = state.time >= params.max_steps_in_episode
        done_steps = jnp.where(
            done_steps, 1 - done, jnp.zeros_like(done)
        )
        return done, done_steps.astype(float) # jnp.logical_or(done_steps, state.terminal)
