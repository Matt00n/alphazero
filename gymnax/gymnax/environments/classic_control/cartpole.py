...
class EnvParams:
    ...
    max_steps_in_episode: int = 50000  # v0 had only 200 steps!
...
class CartPole(environment.Environment):
    ...
    def step_env(
        ...
        prev_terminal, prev_truncated = self.is_terminal(state, params)
        ...
        reward = 1.0 - jnp.maximum(prev_terminal, prev_truncated)

        # Update state dict and evaluate termination conditions
        state = EnvState(x, x_dot, theta, theta_dot, state.time + 1)
        terminated, truncated = self.is_terminal(state, params)
    ...
    def reset_env(
        ...
        # state.info['truncation'] = jnp.zeros(key.shape[:-1]) # Does this work ?
    ...
    def is_terminal(self, state: EnvState, params: EnvParams) -> bool:
        ...
        terminated = jnp.logical_or(done1, done2).astype(float)
        # terminated = jnp.where(
        #     terminated, jnp.ones(1), jnp.zeros(1)
        # ).astype(float)
        truncated = jnp.where(
            state.time >= params.max_steps_in_episode, 1 - terminated, jnp.zeros_like(terminated)
        )
