...
from mctx_dist._src import action_selection
from mctx_dist._src import base
from mctx_dist._src import tree as tree_lib
...
def expand(
  ...
  chex.assert_shape(step.value, [batch_size, tree.num_value_atoms])
...
def instantiate_tree_from_root(
  ...
  num_value_atoms = root.value.shape[-1]
  chex.assert_shape(root.value, [batch_size, num_value_atoms])
  num_nodes = num_simulations + 1
  data_dtype = root.value.dtype
  batch_node = (batch_size, num_nodes)
  batch_node_action = (batch_size, num_nodes, num_actions)
  batch_node_value = (batch_size, num_nodes, num_value_atoms)
  ...
  tree = Tree(
      ...
      raw_values=jnp.zeros(batch_node_value, dtype=data_dtype),
      node_values=jnp.zeros(batch_node_value, dtype=data_dtype),
      ...
      children_values=jnp.zeros((batch_size, num_nodes, num_actions, num_value_atoms), dtype=data_dtype),
